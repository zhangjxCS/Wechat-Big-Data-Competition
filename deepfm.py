...
if __name__ == "__main__":
    # 每个行为的epoch
    action_epoch = {"read_comment": int(sys.argv[5]), "like": int(sys.argv[6]), "click_avatar": int(sys.argv[7]), "forward": int(sys.argv[8])}
    submit = pd.read_csv(ROOT_PATH + '/test_data.csv')[['userid', 'feedid']]
    for action in ACTION_LIST:
        USE_FEAT = ['userid', 'feedid', action] + FEA_FEED_LIST[1:] + [f'embed{b}' for b in range(int(sys.argv[1]))]
        ...
        dense_features = ['videoplayseconds'] + [f'embed{b}' for b in range(int(sys.argv[1]))]
        ...
        for feat in sparse_features:
            if feat not in ['manual_keyword_list', 'machine_keyword_list', 'manual_tag_list', 'machine_tag_list']:
                lbe = LabelEncoder()
                data[feat] = lbe.fit_transform(data[feat])
            elif feat == 'machine_tag_list':
                data[feat] = data[feat].apply(machine_tag_process)
                lbe = LabelEncoder()
                data[feat] = lbe.fit_transform(data[feat])
            elif feat in ['manual_keyword_list', 'machine_keyword_list', 'manual_tag_list']:
                data[feat] = data[feat].apply(keyword_tag_process)
                lbe = LabelEncoder()
                data[feat] = lbe.fit_transform(data[feat])
        ...
        fold = int(sys.argv[9])
        before = int((fold-1)*0.1*train.shape[0])
        after = int(fold*0.1*train.shape[0])
        valid = train.iloc[before:after].reset_index(drop=True)
        train = pd.concat([train.iloc[:before], train.iloc[after:]]).reset_index(drop=True)
        valid_model_input = {name: valid[name] for name in feature_names}
        valid_label = valid[target].values
        ...
        model = MyDeepFM(linear_feature_columns=linear_feature_columns, dnn_feature_columns=dnn_feature_columns,
                        task='binary',
                        l2_reg_linear=float(sys.argv[2]), l2_reg_embedding=float(sys.argv[3]), l2_reg_dnn=float(sys.argv[4]),
                        device=device)

        model.compile("adagrad", "binary_crossentropy", metrics=["binary_crossentropy", "auc"])

        history = model.fit(train_model_input, train[target].values, batch_size=512, epochs=action_epoch[action], verbose=1,
                            validation_data=[valid_model_input,valid_label])
        pred_ans = model.predict(test_model_input, 128)
        submit[action] = pred_ans
        torch.cuda.empty_cache()
    # 保存提交文件
    submit.to_csv(f"./submit_base_deepfm_{int(sys.argv[9])}valid_b榜.csv", index=False)